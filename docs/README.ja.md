...
| 📎     | Pull Requestリンク — 角括弧で状態を表示（`[Open]`/`[Draft]`/`[Merged]`/`[Closed]`、色分け）、チェックが存在する場合は括弧内に色分けされた CI サマリー（`(N passed)`/`(N running)`/`(N failed)`）も表示 |
...
- **ファイルツリー**: 左右配置と flatten（空ディレクトリの折りたたみ）の切り替え
