...
| オプション                     | 説明                                                              | デフォルト |
| ------------------------------ | ----------------------------------------------------------------- | :--------: |
| [`--plan <plan>`](#プラン選択) | サブスクリプションプランのコスト制限を設定 (pro, max5x, max20x) |   `pro`    |
...
- **ブロック使用量**: 5時間ブロックのコスト使用量とパーセンテージ
...
> [!NOTE]
> Anthropicはサブスクリプション使用量の計算式を公開していません。ブロック使用量はAPI公示価格に基づく推定値であり、実際の`/usage`の値と異なる場合があります。

### 動作方法

`~/.claude/projects/`のJSONLファイルを自動的にパースして検出：

1. **5時間ビリングブロック** - 累積時間と非アクティブギャップに基づくブロック境界検出（リセットタイマーは時間単位でfloor）
2. **コスト計算** - モデル別価格（opus/sonnet/haiku）× トークン数でコストを算出
3. **クロスプロジェクトスキャン** - `~/.claude/projects/`配下の全プロジェクトをスキャン（ブロックはプロジェクト間で統合）
...
プランはmacOS Keychain（`Claude Code-credentials` → `rateLimitTier`）から**自動検出**されるため、設定は不要です。

> [!NOTE]
> 自動検出は**macOS専用**です。他のプラットフォームでは`--plan`フラグでプランを明示的に指定してください。

手動で指定するには、`--plan`フラグを使用してください：
