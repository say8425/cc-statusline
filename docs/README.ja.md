...
4. **週間リセットタイマー** - 週間制限リセット時刻（`rate_limits.seven_day.resets_at`）、`MM/DD(曜日) HH:MM`形式。曜日名は `LC_ALL` / `LC_TIME` / `LANG` で決まるロケール準拠（例：`ja_JP.UTF-8` → `02/15(木) 17:00`、`en_US.UTF-8` → `02/15(Thu) 17:00`）。3つとも未設定ならランタイム既定のロケール（現行の Bun では `en-US`）にフォールバック — macOS のターミナルは「Set locale environment variables on startup」が無効だと `LANG` が空のまま
