...
- **Sidebar view**: switch between a folder tree and a flat file list (the **Flatten** toggle lists every changed file by full path — no folders — with a filter box; handy since the tree's folder-collapsing rarely has anything to collapse); the tree also has a left/right placement toggle
