...
- 사이드바 뷰 전환 — 폴더 트리(기본) / 플랫 목록. `Flatten` 토글(라벨은 Flatten이지만 동작은 폴더 트리를 플랫 목록으로 펼치는 것; `⋯` 메뉴, localStorage `cc-statusline:file-view`=tree|list, 기본 tree)로 전환. **트리**는 `@pierre/trees` FileTree(단일 자식 폴더 체인만 `flattenEmptyDirectories:true`로 접음 — 실제 diff엔 그런 체인이 드물어 시각 효과가 거의 없음). **플랫 목록**은 `src/viewer/flatList.ts`의 `renderFlatList`가 그리는 커스텀 리스트로, 모든 변경 파일을 폴더 계층 없이 전체 경로 한 줄씩 나열(상태 배지 A/U/M/D/R + 색, 상단 `Filter files…` 부분일치 필터, 행 클릭 시 `codeView.scrollTo`). @pierre/trees엔 전체 플랫 모드가 없어(`flattenEmptyDirectories`는 단일 자식 체인 전용) 리스트는 직접 렌더. 두 뷰 모두 파일 세트(`treeKey`)가 바뀔 때만 재생성해 트리 확장/리스트 필터·스크롤을 보존. 토글 시엔 반대 뷰로 교체(cleanUp 후 재생성)
