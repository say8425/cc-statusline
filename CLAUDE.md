...
- diffdeck 버전 검증은 3층이고 **마지막 층은 의도적으로 비워 뒀다**: ① `diff-contract.test.ts`가 실제 설치본을 ephemeral port + 격리 `XDG_CACHE_HOME`에 spawn해 pid+version 대조 → *실행 중인 데몬 = 설치본*, ② 같은 파일의 range 단언(`Bun.semver.satisfies`) → *설치본 ∈ 매니페스트 range*, ③ *설치본 = lockfile resolved* → **검사 안 함**. ③은 로컬에서만 생긴다(CI는 `--frozen-lockfile`이라 installed = lockfile 보장). 주 원인은 git 브랜치 전환 — node_modules는 git이 관리하지 않으니 dep 버전이 다른 브랜치를 오가면 자동으로 어긋난다. 닫으려면 `bun list` 출력(이건 node_modules가 아니라 **lockfile**을 읽는다, 실측)을 정규식 파싱해야 하는데, `bun.lock`은 trailing comma JSONC라 `JSON.parse`·`Bun.file().json()`이 둘 다 실패하고 `bun list --json`도 미지원이라 취약한 파싱이 강제된다. 막는 대상이 로컬 오검증뿐이라 **won't fix**. `bun install --frozen-lockfile --dry-run`은 이 드리프트를 감지하지 못한다(exit 0, 실측) — `--frozen-lockfile`은 lockfile↔package.json만 본다. 실제 피해 전례: PR #62의 "1.2.0 계약을 검증했다"는 주장은 node_modules가 1.0.0으로 드리프트한 상태에서 나온 것이었다. 그 부류는 이제 ②가 잡는다
- **CI가 검증하는 diffdeck 버전 ≠ 사용자가 실행하는 버전** (문서화된 리스크 — 테스트로 만들지 말 것): 배포 tarball은 `files: ["dist/index.js"]`라 lockfile을 담지 않아 사용자 `bunx`는 range 내 **최신**을 resolve한다. 지금도 그렇다 — 배포된 5.0.0의 `^1.2.0` → 사용자는 1.3.0을 받는다. 반면 CI는 lockfile 버전으로 계약을 검증한다. 게다가 이 wire contract(`x-diffdeck` 헤더·`bin.diffdeck`·CLI 플래그·토큰 경로)는 diffdeck 입장에선 내부 구현이라 **semver가 보호하지 않는다** — 헤더가 실제로 바뀐 전례가 `src/diff-server/ensure.ts:56-57`의 `x-cc-statusline` 폴백으로 남아 있고, `maybeSpawn`이 fire-and-forget이라 깨져도 조용하다. 그럼에도 range-max를 CI에서 설치하는 테스트는 만들지 않는다: 네트워크 의존적·비결정적이라 이 저장소와 무관한 diffdeck 릴리스가 CI를 빨갛게 만든다. dep은 수동 범프라 **범프 시점이 곧 lockfile = range-max 시점**이고 그때 `diff-contract.test.ts`가 검증한다. 대신 범프 전에 diffdeck 릴리스 노트를 읽고 뷰어 외 변경(CLI 플래그·핑 헤더·토큰 경로·bin 경로)이 보이면 계약을 직접 확인할 것
