...
│   │   ├── flatList.ts             # 플랫 파일 목록 (filterFlatFiles·flatFileStatusMeta·renderFlatList)
│   │   └── search/
...
│       ├── viewer-flat-list.test.ts    # flatList(필터·상태 메타) 테스트
│       ├── viewer-prefs.test.ts        # prefs(readTreeSide·readFileView) 테스트
...
- diff 뷰어 툴바는 좌(모드·Unified/Split·Refresh)/우(상태·검색·⋯ 오버플로) 클러스터로 정렬; 저중요 컨트롤(Include untracked·Watch·Flat list·파일트리 좌/우)은 `⋯` 오버플로 메뉴에 위치. Unified/Split은 Pierre식 세그먼트 그룹(`.tb-seg`, `aria-pressed`가 CSS 활성 조각(#2a2a2c)과 접근성 겸용)으로 현재 모드가 항상 표시되고, Refresh는 아이콘 버튼(인라인 rotate-cw SVG, aria-label 유지). 검색(`#find-open`, 돋보기 아이콘)은 클릭 시 find bar를 여는(=Cmd/Ctrl+F) 발견성 버튼 — find bar가 열려 있는 동안 `:has()` CSS로 숨김
- 사이드바 뷰 전환 — 폴더 트리(기본) / 플랫 목록. `Flat list` 토글(`⋯` 메뉴, localStorage `cc-statusline:file-view`=tree|list, 기본 tree)로 전환. **트리**는 `@pierre/trees` FileTree(단일 자식 폴더 체인만 `flattenEmptyDirectories:true`로 접음 — 실제 diff엔 그런 체인이 드물어 시각 효과가 거의 없음). **플랫 목록**은 `src/viewer/flatList.ts`의 `renderFlatList`가 그리는 커스텀 리스트로, 모든 변경 파일을 폴더 계층 없이 전체 경로 한 줄씩 나열(상태 배지 A/U/M/D/R + 색, 상단 `Filter files…` 부분일치 필터, 행 클릭 시 `codeView.scrollTo`). @pierre/trees엔 전체 플랫 모드가 없어(`flattenEmptyDirectories`는 단일 자식 체인 전용) 리스트는 직접 렌더. 두 뷰 모두 파일 세트(`treeKey`)가 바뀔 때만 재생성해 트리 확장/리스트 필터·스크롤을 보존. 토글 시엔 반대 뷰로 교체(cleanUp 후 재생성)
- 파일트리 좌/우 배치(`data-tree-side`, localStorage `cc-statusline:tree-side`, 기본 left) 토글 가능. 파일트리 상단 padding은 diff의 8px(`--diffs-gap-fallback`)와 일치
