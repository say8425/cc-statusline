...
- TypeScript는 7.x(네이티브 Go 구현)를 쓴다. **`tsserver`가 없다** — TS7 패키지의 `lib/`엔 `tsc.js`·`getExePath.js`뿐이고 `bin`도 `tsc` 하나다(6.x엔 `tsserver.js`·`tsserverlibrary.js`가 있었다). `typecheck` 게이트는 `tsc --noEmit`만 쓰므로 CI·빌드엔 무관하고, 이 저장소엔 에디터 설정 파일도 없어 기본값(에디터 번들 TS)이면 영향이 없다. 다만 개인 설정에서 "workspace TypeScript"를 고르고 있다면 가리킬 `tsserver`가 없으니 그때 조정할 것. 린터는 무관하다 — `oxlint-tsgolint`는 `typescript` 패키지에 의존하지 않는 자체 네이티브 바이너리다(`dependencies` 없음, `@oxlint-tsgolint/<platform>`만 optional)
