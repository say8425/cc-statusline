...
- 주간 리셋 시간 (7일 사용량 리셋 시각, `MM/DD(요일) HH:MM`) — 요일 이름만 로케일을 타고 나머지 자릿수 표기는 고정이다(`ko_KR.UTF-8`이면 `⏰ 09/11(금) 07:00`, `en_US.UTF-8`이면 `⏰ 09/11(Fri) 07:00`). 폭이 로케일마다 조금 달라지지만(`zh_CN`은 `(周五)`로 2글자) 한 머신 안에선 일정하다
...
- **diffdeck은 캐럿 없이 정확 핀한다** — 핀된 값은 `package.json`의 `dependencies`에서 읽을 것. **이 문서에 버전 번호를 적지 않는 건 의도적이다**: 하드코딩하면 범프마다 같이 고쳐야 하는데, Dependabot이 여는 `fix(deps):` PR은 `package.json`·`bun.lock`만 건드리므로 문서는 남겨진 채 조용히 틀려진다. 실제로 그렇게 됐다 — 이 문단이 `1.3.0`을 적고 있는 동안 매니페스트는 [#78](https://github.com/say8425/cc-statusline/pull/78)로 1.3.1, [#84](https://github.com/say8425/cc-statusline/pull/84)로 1.4.0까지 갔고 아무도 눈치채지 못했다. 문서가 틀려도 CI는 초록인데, `diff-contract.test.ts`는 매니페스트·lockfile·설치본만 대조할 뿐 산문은 읽지 않기 때문이다. 이 문서의 다른 곳에 남은 버전 번호들은 이미 일어난 일에 대한 과거형 기록이라 낡지 않지만, *현재* 핀이 무엇인지는 어디에도 적지 말 것 — 썩는 건 현재형 주장뿐이다. 배포 tarball은 `files: ["dist/index.js"]`라 lockfile을 담지 않으므로, 배포된 `dependencies`가 곧 사용자 `bunx`의 resolve 기준이다 — 핀은 사용자에게 그대로 전파되고, 그래서 **CI가 계약을 검증한 바로 그 버전이 사용자 머신에서 돈다** — 단 bunx 캐시가 새로 채워지는 시점부터다(위의 캐시 항목 참고: 릴리스가 살아 있는 캐시를 자동으로 갱신하지 않는다). 캐럿이던 시절엔 이 둘이 구조적으로 어긋났다(배포된 5.0.0의 `^1.2.0` → 사용자는 1.3.0을 받았다). 핀하는 이유는 이 wire contract가 diffdeck 입장에선 내부 구현이라 **semver가 보호해주지 않기** 때문이다: `ensure.ts`의 `probeServer` 주석이 기록하듯 pre-0.2.2 diffdeck은 버전 헤더를 아예 보내지 않았는데, 0.2.2는 **패치** 릴리스였다(0.x라 semver가 원래 약속하지 않는 구간이긴 하다 — 다만 근거의 핵심은 버전 번호가 아니라 이 wire contract가 diffdeck 입장에선 내부 구현이라는 점이므로 1.x에서도 보호받지 못한다). 게다가 `maybeSpawn`이 fire-and-forget이라 깨져도 조용하다. (`probeServer`가 아직 인식하는 `x-cc-statusline` 마커는 diffdeck의 변경이 아니라 이 저장소가 뷰어를 분리하며 남긴 자기 흔적이니, 상류 드리프트의 근거로 쓰지 말 것.)
- **핀의 대가와 범프 절차**: diffdeck 버그픽스가 자동으로 흘러오지 않으므로 **cc-statusline 릴리스를 거쳐야 사용자에게 닿는다**. 범프를 시작시키는 건 **Dependabot**이다(`.github/dependabot.yml`, cron으로 매일 09시 KST, diffdeck만 감시 — `daily`는 평일만 돌아 주말 릴리스를 놓치므로 일부러 cron이다). **`cooldown: default-days: 0`이 필수다**: 명시하지 않으면 updater가 기본 3일 cooldown을 걸어 갓 나온 버전을 **3일간 미룬다**(영구 제외가 아니라 지연이다 — 그대로 뒀어도 3일 뒤엔 열렸을 것). 실제로 겪었다 — 1.3.1 배포 10시간 뒤 job이 돌았는데 `Filtered out 2 versions due to cooldown` / `Latest version is 1.2.0`으로 끝나 PR이 안 열렸고, **job은 success라 실패 흔적이 어디에도 없었다**. 릴리스가 났는데 PR이 안 보이면 Actions의 `Dependabot Updates` 워크플로 로그부터 볼 것(`gh run view <id> --log`) — 그 사례가 [run 30773993483](https://github.com/say8425/cc-statusline/actions/runs/30773993483)이고, 옵션 전체는 [Dependabot options reference](https://docs.github.com/en/code-security/reference/supply-chain-security/dependabot-options-reference)에 있다 — 핀 때문에 저절로 올라오지 않는 걸 대신 발견해 `fix(deps):` PR을 연다(그 prefix라야 release-please가 patch를 낸다). 그 PR에서 사람이 할 일은 ① diffdeck 릴리스 노트를 읽고 뷰어 외 변경(CLI 플래그·핑 헤더·토큰 경로·`bin` 경로)이 있는지 보고, ② 있으면 계약을 손으로 확인하는 것이다. `package.json`·`bun.lock` 갱신은 Dependabot이 이미 해 둔다. **기계가 대신 해 주는 부분**: PR CI가 `--frozen-lockfile`로 새 버전을 설치한 뒤 `diff-contract.test.ts`를 돌리므로, 계약이 깨졌으면 초록이 아니라 빨강으로 드러난다(정확 핀이라 ②층 range 단언은 설치본과 핀이 같은 한 값을 비교하는 꼴이 되어 통과하고, 다른 버전이 설치돼 있으면 걸린다). 다만 **뷰어 UX 변경은 테스트가 못 보므로** ①은 여전히 사람 몫이다. **range-max를 CI에서 설치하는 테스트는 만들지 않는다** — 네트워크 의존적·비결정적이라 이 저장소와 무관한 diffdeck 릴리스가 PR을 막는다. 핀 이후로는 range-max라는 개념 자체가 없어져 이 논거가 더 분명해졌다
- **`dependabot.yml`이 조용히 죽는 두 가지 방식** (둘 다 2026-08-22에 실측, [#83](https://github.com/say8425/cc-statusline/pull/83)). 위 cooldown 사례와 달리 **Actions 로그에 아무것도 안 남으므로** 증상만 보면 구분이 안 된다 — "릴리스가 났는데 PR이 없다"가 셋 다 같은 모양이다. 먼저 `Dependabot Updates` **실행 기록 자체가 있는지**부터 보고 갈래를 타라: 실행은 있는데 PR이 없으면 cooldown/버전 해석 문제(로그를 읽는다), **실행이 통째로 없으면** 아래 둘 중 하나다. 할 일 없는 틱도 `No update needed`를 남기며 실행 기록이 남으므로, 실행 부재는 유휴가 아니라 **미dispatch**라는 뜻이다.
  - **① 잘못된 키 하나가 파일 전체를 무효화한다.** 스키마의 `/definitions/update`가 `additionalProperties: false`라 모르는 키가 하나라도 있으면 그 키만 무시되는 게 아니라 **config 전체가 파싱 거부**되어 cron·cooldown·allow·commit-message가 전부 죽는다. [#82](https://github.com/say8425/cc-statusline/pull/82)가 `update-types`를 `updates` 항목 직속에 넣어 이 상태를 만들었다(`The property '#/updates/0/' contains additional properties ["update-types"] outside of the schema when none are allowed`). 이 신호는 **커밋의 `dependabot` 체크 런에만** 뜨고, 그것도 **기본 브랜치에서 파일이 바뀐 커밋에만** 붙는다 — PR 브랜치엔 아예 생성되지 않으므로(실측: #82의 헤드 `49e008f`엔 0개, main의 `7f19f50`·`169b734`엔 1개씩) **CI 게이트로 쓸 수 없다**. 방어는 둘뿐이다: 푸시 전 로컬에서 SchemaStore 스키마의 허용 키 집합과 대조하고, 머지 직후 main에서 확인한다 — `gh api repos/say8425/cc-statusline/commits/main/check-runs --jq '.check_runs[]|select(.app.slug=="dependabot")'`. 참고로 `update-types`는 `allow`/`ignore`/`groups` **하위** 키이고(문서 앵커부터 `#update-types-allow`다), `allow` 밑에서도 넣을 이유가 없다 — 생략이 곧 전 타입 허용이라 이 옵션은 오직 *좁히기*만 한다("최신 추종"을 위해 추가하는 건 무의미). 값도 `allow`/`ignore`에선 `version-update:semver-major` 형태이고 bare `major`는 `groups` 전용 어휘라, 위치만 고쳐 들여써도 여전히 거부된다.
  - **② 열린 PR이 슬롯을 물면 스케줄 실행이 dispatch되지 않는다.** `open-pull-requests-limit`이 찬 상태의 틱은 `limit reached` 로그조차 없이 통째로 사라진다. 실측 2/2, 예외 0건 — 08-04 틱 없음(#78이 08-03T02:57~08-04T02:19 열림), 08-13~08-22 틱 전부 없음(#80이 08-12T00:07~ 열림). 이 탓에 diffdeck 1.4.0 배포(08-21T12:37Z) 직후의 08-22T00:00Z 틱이 날아갔고, "PR이 1.3.2에 멈춰 있다"로 보였다. **그래서 상한을 3으로 뒀다** — 감시 대상이 diffdeck 하나뿐이라 PR 폭증 위험은 없고, 스테일 PR 하나가 다음 버전을 영구히 막는 구조를 없애는 안전밸브다. 1로 되돌리지 말 것. 크론을 기다리지 않고 즉시 재해석시키려면 해당 PR에 `@dependabot recreate` 코멘트를 단다(단 ①이 살아 있으면 이 트리거도 무력하다 — 트리거된 job도 `dependabot.yml`을 읽기 때문이다).
