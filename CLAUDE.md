...
- PR 상태 배지: `📎` 라벨 옆에 `[Open]`/`[Draft]`/`[Merged]`/`[Closed]` 상태(색상별: 녹색/흰색/보라/빨강)와 CI 체크 집계를 `(N passed)`/`(N running)`/`(N failed)` 텍스트로 표시(색상별: 녹색/노랑/빨강, PR 상태와 무관하게 체크가 있으면 항상 표시). `gh pr view --json url,state,isDraft,statusCheckRollup`, 집계 로직은 `src/git/ciStatus.ts`
